...
VENDOR_DIR="$ROOT_DIR/vendor/bash"; BUILD_DIR="$ROOT_DIR/build"; PATCH_DIR="$ROOT_DIR/src/bash_patch"
[[ -f "$VENDOR_DIR/.fetch_complete" ]]||exit 1
shopt -s nullglob; patches=("$PATCH_DIR"/*.patch); shopt -u nullglob
(( ${#patches[@]} > 0 ))||exit 1
mkdir -p "$BUILD_DIR"; cd "$VENDOR_DIR"
